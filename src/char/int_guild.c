...
struct guild * inter_guild_fromsql(int guild_id)
{
	...
	{	// Fix reduction of MAX_GUILD [PoW]
		ShowWarning("Clã %d:%s especifica capacidade (%d) maior que MAX_GUILD (%d)\n", guild_id, g->name, g->max_member, MAX_GUILD);
...
int inter_guildcastle_tosql(struct guild_castle *gc)
{
	...
	if (SQL_ERROR == Sql_Query(sql_handle, StringBuf_Value(&buf)))
		...
		ShowInfo("Castelo de clã salvo (%d)\n", gc->castle_id);
...
static struct guild_castle* inter_guildcastle_fromsql(int castle_id)
{
	...
	if (save_log)
		ShowInfo("Castelo de clã carregado (%d - clã %d)\n", castle_id, gc->guild_id);
...
static bool exp_guild_parse_row(char* split[], int column, int current)
{
	...
	if (exp < 0 || exp >= INT_MAX) {
		ShowError("exp_guild: Exp inválido %d na linha %d\n", exp, current);
		return false;
	}
...
int inter_guild_CharOnline(int char_id, int guild_id)
{
   ...
   int i;
	...
	if(!g) {
		ShowError("Clã %d do personagem %d não encontrado!\n", guild_id, char_id);
		return 0;
	}
...
int guild_calcinfo(struct guild *g)
{
	...
	if(g->max_member > MAX_GUILD)
	{
		ShowError("Clã %d:%s tem capacidade para muitos membros (%d), máx. suportado é %d\n", g->guild_id, g->name, g->max_member, MAX_GUILD);
		g->max_member = MAX_GUILD;
	}
...
int mapif_guild_noinfo(int fd,int guild_id)
{
	...
	ShowWarning("int_guild: informação não encontrada %d\n",guild_id);
	if(fd<0)
		mapif_sendall(buf,8);
	else
		mapif_send(fd,buf,8);
...
int mapif_guild_withdraw(int guild_id,int account_id,int char_id,int flag, const char *name, const char *mes)
{
	...
	ShowInfo("int_guild: retirada de clã (%d - %d: %s - %s)\n",guild_id,account_id,name,mes);
	return 0;
}
...
int mapif_parse_CreateGuild(int fd,int account_id,char *name,struct guild_member *master)
{
	...
	if(search_guildname(name) != 0){
		ShowInfo("int_guild: clã com o mesmo nome já existe [%s]\n",name);
	...
	if (!inter_guild_tosql(g,GS_BASIC|GS_POSITION|GS_SKILL|GS_MEMBER)) {
		...
		ShowError("Falha em criar clã %s (Mestre do clã: %s)\n", g->name, g->master);
		mapif_guild_created(fd,account_id,NULL);
		aFree(g);
		return 0;
	}
	ShowInfo("Clã criado %d - %s (Mestre do clã: %s)\n", g->guild_id, g->name, g->master);
	...
	if(log_inter)
		inter_log("clã %s (id=%d) criado por mestre %s (id=%d)\n",
...
int mapif_parse_GuildMemberInfoChange(int fd,int guild_id,int account_id,int char_id,int type,const char *data,int len)
{
	...
	// Not Found
	if(i==g->max_member){
		ShowWarning("int_guild: GuildMemberChange: Não encontrado %d,%d no clã (%d - %s)\n",
...
int inter_guild_charname_changed(int guild_id,int account_id, int char_id, char *name)
{
	...
	if( g == NULL )
	{
		ShowError("inter_guild_charrenamed: Não pôde encontrar clã %d.\n", guild_id);
		return 0;
	}
	...
	if( i == g->max_member )
	{
		ShowError("inter_guild_charrenamed: Não foi possível encontrar personagem %d no clã\n", char_id);
		return 0;
	}
...
int mapif_parse_GuildCastleDataSave(int fd, int castle_id, int index, int value)
{
	...
	if (gc == NULL) {
		ShowError("mapif_parse_GuildCastleDataSave: castelo id=%d não encontrado\n", castle_id);
		return 0;
	}
	...
	switch (index) {
		case 1:
			if (log_inter && gc->guild_id != value) {
				...
				inter_log("clã %s (id=%d) %s castelo id=%d\n",
		...
		default:
			...
			ShowError("mapif_parse_GuildCastleDataSave: índice não encontrado=%d\n", index);
			return 0;
	}
