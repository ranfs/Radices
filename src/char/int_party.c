...
int inter_party_sql_init(void)
{
	...
	if (!party_pt) {
		ShowFatalError("inter_party_sql_init: Sem Mmeória!\n");
		exit(EXIT_FAILURE);
	}
...
static void mapif_party_noinfo(int fd, int party_id, int char_id)
{
	...
	WFIFOSET(fd,12);
	ShowWarning("int_party: informação não encontrada (party_id=%d char_id=%d)\n", party_id, char_id);
}
...
int inter_party_CharOnline(int char_id, int party_id)
{
	...
	p = inter_party_fromsql(party_id);
	if(!p) {
		ShowError("Grupo %d do personagem %d não encontrado!\n", party_id, char_id);
		return 0;
	}
