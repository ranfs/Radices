...
static int auction_end_timer(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	if( (auction = (struct auction_data *)idb_get(auction_db_, id)) != NULL )
	{
		if( auction->buyer_id )
		{
			mail_sendmail(0, "Gerente de leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Obrigado, você ganhou o leilão!.", 0, &auction->item);
			mapif_Auction_message(auction->buyer_id, 6); // You have won the auction
			mail_sendmail(0, "Gerente de leilão", auction->seller_id, auction->seller_name, "Leilão", "Pagamento para seu leilão!.", auction->price, NULL);
		}
		else
			mail_sendmail(0, "Gerente de leilão", auction->seller_id, auction->seller_name, "Leilão", "Nenhum comprador foi encontrado para seu leilão.", 0, &auction->item);
...
static void mapif_parse_Auction_close(int fd)
{
	...
	mail_sendmail(0, "Gerente de leilão", auction->seller_id, auction->seller_name, "Leilão", "Leilão fechou.", auction->price, NULL);
	// Send Item to Buyer
	mail_sendmail(0, "Gerente de leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Ganhador do leilão.", 0, &auction->item);
	mapif_Auction_message(auction->buyer_id, 6); // You have won the auction
	auction_delete(auction);
...
static void mapif_parse_Auction_bid(int fd)
{
	...
	{ // Send Money back to the previous Buyer
		...
		{
			mail_sendmail(0, "Gerente de leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Alguém ofertou um lance maior.", auction->price, NULL);
			mapif_Auction_message(auction->buyer_id, 7); // You have failed to win the auction
		}
		else
			mail_sendmail(0, "Gerente de leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Você ofertou um lance maior.", auction->price, NULL);
	}
	...
	{ // Automatic won the auction
		...
		mail_sendmail(0, "Gerente de leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Você ganhou o leilão.", 0, &auction->item);
		mapif_Auction_message(char_id, 6); // You have won the auction
		mail_sendmail(0, "Auction Manager", auction->seller_id, auction->seller_name, "Auction", "Pagamento para o seu leilão!.", auction->buynow, NULL);
