...
int storage_fromsql(int account_id, struct storage_data* p)
{
	...
	ShowInfo("Carregamento completo do armazém da DB - id: %d (total: %d)\n", account_id, p->storage_amount);
	return 1;
}
...
int guild_storage_tosql(int guild_id, struct guild_storage* p)
{
	...
	ShowInfo ("Armazém do clã salvo na DB - clã: %d\n", guild_id);
	return 0;
}
...
int guild_storage_fromsql(int guild_id, struct guild_storage* p)
{
	...
	ShowInfo("Carregamento completo do armazém do clã da DB - id: %d (total: %d)\n", guild_id, p->storage_amount);
	return 0;
}
...
int mapif_parse_SaveGuildStorage(int fd)
{
	...
	{
		ShowError("inter storage: erro no tamanho das informações %d != %d\n", sizeof(struct guild_storage), len - 12);
	}
	else
	{
		if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `guild_id` FROM `%s` WHERE `guild_id`='%d'", guild_db, guild_id) )
