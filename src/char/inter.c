...
void geoip_readdb(void){
	...
	if(fread(geoip_cache, sizeof(unsigned char), bufa.st_size, db) != bufa.st_size) { ShowError("leitura do geoip_cache não leu todos os elementos.\n"); }
	fclose(db);
	ShowStatus("Leitura finalizada do "CL_GREEN"GeoIP"CL_RESET" Database.\n");
}
/* [Dekamaster/Nightroad] */
...
void mapif_parse_accinfo(int fd) {
	...
	if (account_id < START_ACCOUNT_NUM) {	// is string
		if ( SQL_ERROR == Sql_Query(sql_handle, "SELECT `account_id`,`name`,`class`,`base_level`,`job_level`,`online` FROM `char` WHERE `name` LIKE '%s' LIMIT 10", query_esq)
			...
			if( Sql_NumRows(sql_handle) == 0 ) {
				inter_to_fd(fd, u_fd, aid, "Nenhum resultado encontrado com seus critérios, '%s'",query);
			} else {
				Sql_ShowDebug(sql_handle);
				inter_to_fd(fd, u_fd, aid, "Um erro ocorreu, reporte ao admin.");
			}
			...
			if( Sql_NumRows(sql_handle) == 1 ) {//we found a perfect match
				...
				inter_to_fd(fd, u_fd, aid, "Sua consulta retornou os seguintes resultados %d, seja mais específico...",(int)Sql_NumRows(sql_handle));
	...
	if( account_id ) {
		...
		if ( SQL_ERROR == Sql_Query(sql_handle, "SELECT `userid`, `user_pass`, `email`, `last_ip`, `group_id`, `lastlogin`, `logincount`, `state` FROM `login` WHERE `account_id` = '%d' LIMIT 1", account_id)
			|| Sql_NumRows(sql_handle) == 0 ) {
			if( Sql_NumRows(sql_handle) == 0 ) {
				inter_to_fd(fd, u_fd, aid,  "Nenhuma conta com ID '%d' foi encontrado.", account_id );
			} else {
				inter_to_fd(fd, u_fd, aid, "Um erro ocorreu, reporte ao admin.");
				Sql_ShowDebug(sql_handle);
			}
		...
		inter_to_fd(fd, u_fd, aid, "Usuário: %s | Grupo de GM: %d | Status: %d", userid, level, state );
		...
		inter_to_fd(fd, u_fd, aid, "Este usuário entrou %d vezes, a última vez foi em %s", logincount, lastlogin );
		...
		if ( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`, `name`, `char_num`, `class`, `base_level`, `job_level`, `online` FROM `char` WHERE `account_id` = '%d' ORDER BY `char_num` LIMIT %d", account_id, MAX_CHARS)
				...
				if( Sql_NumRows(sql_handle) == 0 )
					inter_to_fd(fd, u_fd, aid,"Essa conta não possui personagens.");
				else {
					inter_to_fd(fd, u_fd, aid,"Um erro ocorreu, reporte ao admin.");
					Sql_ShowDebug(sql_handle);
				}
...
int inter_accreg_tosql(int account_id, int char_id, struct accreg* reg, int type)
{
	...
	case 2: //Account Reg
		...
		break;
	case 1: //Account2 Reg
		ShowError("inter_accreg_tosql: O cahr-server não deveria manipular dados tipo 1 (##). Isso é trabalho do login-server!\n");
		return 0;
	default:
...
int inter_accreg_fromsql(int account_id,int char_id, struct accreg *reg, int type)
{
	...
	case 2: //account reg
		...
		break;
	case 1: //account2 reg
		ShowError("inter_accreg_fromsql: O char-server não deveria manipular dados tipo 1 (##). Isso é trabalho do login-server!\n");
		return 0;
	default:
...
int inter_init_sql(const char *file)
{
	...
	ShowInfo("Conectando à DB de personagens.... (char-server)\n");
...
int mapif_account_reg_reply(int fd,int account_id,int char_id, int type)
{
	...
	if(reg->reg_num==0){
		...
		WFIFOW(fd,2)=p;
		if (p>= 5000)
			ShowWarning("Muitas acc regs para %d:%d, nem todos os valores foram carregados.\n", account_id, char_id);
	}
...
int mapif_parse_WisRequest(int fd)
{
	...
	if (RFIFOW(fd,2)-52 >= sizeof(wd->msg)) {
		ShowWarning("inter: Mensagem wis muito longa.\n");
		return 0;
