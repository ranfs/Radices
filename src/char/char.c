...
void set_char_online(int map_id, int char_id, int account_id)
{
	...
	{
		ShowNotice("set_char_online: Personagem %d:%d marcado no map-server %d, mas o map-server %d disse ter (%d:%d) online!\n",
...
int memitemdata_to_sql(const struct item items[], int max, int id, int tableswitch)
{
	...
	default:
		ShowError("Nome de tabela inválido!\n");
		return 1;
	}
...
int mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	if (save_log) ShowInfo("Requisição de carregamento de personagem (%d)\n", char_id);
	...
	{
		ShowError("Requisitado personagem não existente: %d!\n", char_id);
		SqlStmt_Free(stmt);
		return 0;
	}
	p->last_point.map = mapindex_name2id(last_map);
	...
	for( i = 0; i < MAX_SKILL && SQL_SUCCESS == SqlStmt_NextRow(stmt); ++i )
	{
		if( tmp_skill.id < ARRAYLENGTH(p->skill) )
			...
			ShowWarning("mmo_char_fromsql: ignorando habilidade inválida (id=%u,lv=%u) do personagem %s (AID=%d,CID=%d)\n", tmp_skill.id, tmp_skill.lv, p->name, p->account_id, p->char_id);
	}
	strcat(t_msg, " skills");
	...
	while( SQL_SUCCESS == SqlStmt_NextRow(stmt) )
	{
		if( hotkey_num >= 0 && hotkey_num < MAX_HOTKEYS )
			...
			ShowWarning("mmo_char_fromsql: ignorando hotkey inválida (hotkey=%d,tipo=%u,id=%u,lv=%u) do personagem %s (AID=%d,CID=%d)\n", hotkey_num, tmp_hotkey.type, tmp_hotkey.id, tmp_hotkey.lv, p->name, p->account_id, p->char_id);
	}
...
int delete_char_sql(int char_id)
{
	...
	{
		ShowError("delete_char_sql: impossível de obter dados do personagem, deleção abortada.\n");
	...
	if( ( char_del_level > 0 && base_level >= char_del_level )
	 || ( char_del_level < 0 && base_level <= -char_del_level )
			...
			ShowInfo("Deleção de personagem abortada: %s, BaseLevel: %i\n", name, base_level);
			return -1;
	}
...
{
	ShowWarning("Conexão ao login-server perdida.\n\n");
}
...
int parse_fromlogin(int fd) {
	...
	if( fd != login_fd ) {
		ShowDebug("parse_fromlogin: Desconectando sessão inválida #%d (não é o login-server)\n", fd);
		do_close(fd);
		return 0;
	}
	...
	while(RFIFOREST(fd) >= 2) {
		...
		case 0x2711:
			...
			if (RFIFOB(fd,2)) {
				...
				ShowError("Não pôde conectar ao login-server.\n");
				ShowError("As senhas de comunicação entre servidores (padrão s1/p1) estão provavelmente inválidas.\n");
				ShowError("Além disso, confirme se o login_db tem os corretos logins/senhas e o sexo da conta é S.\n");
				ShowError("As senhas de comunicação são definidas em map_athena.conf e char_athena.conf\n");
				set_eof(fd);
				return 0;
			} else {
				ShowStatus("Conectado ao login-server (conexão #%d).\n", fd);
				loginif_on_ready();
			}
			RFIFOSKIP(fd,3);
		break;
		...
		default:
			ShowError("Pacote 0x%04x desconhecido recebido do login-server, desconectando.\n", command);
			set_eof(fd);
			return 0;
		}
	}
...
{
	ShowStatus("O map-server #%d desconectou-se.\n", id);
	mapif_server_reset(id);
}
...
int parse_frommap(int fd)
{
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x2afa: // Receiving map names list from the map-server
			...
			ShowStatus("Map-Server %d conectado: %d mapas, do IP %d.%d.%d.%d porta %d.\n",
			...
			int x;
			if (j == 0) {
				ShowWarning("Map-server %d NÃO tem mapas.\n", id);
			} else {
				...
				return 0;
		{
			...
			if( Sql_NumRows(sql_handle) > 0 )
			{
				...
				if (count >= 50)
					ShowWarning("Muitas mudanças de status de %d:%d, alguns deles não foram carregados.\n", aid, cid);
				if (count > 0)
				{
		...
		case 0x2afe: //set MAP user count
			...
			if (RFIFOW(fd,2) != server[id].users) {
				...
				ShowInfo("Contagem de usuários: %d (Servidor: %d)\n", server[id].users, id);
			}
			RFIFOSKIP(fd, 4);
			break;
				...
				return 0;
		{
			...
			for(i = 0; i < server[id].users; i++) {
				...
				{
					ShowNotice("Set map user: Personagem (%d:%d) foi marcado no map-server %d, porém o map-server %d diz ter (%d:%d) online!\n",
				...
				return 0;
		{
			...
			{
				ShowError("parse_from_map (save-char): Tamanho imcompatível! %d != %d\n", size-13, sizeof(struct mmo_charstatus));
				RFIFOSKIP(fd,size);
				break;
			}
				...
				character->char_id == cid))
			{
				...
				ShowError("parse_from_map (save-char): Recebida informação de personagem não existente/offline. (%d:%d).\n", aid, cid);
...
//					result = 2; // 2-gm level too low
		...
		case 0x2736: // ip address update
			...
			ShowInfo("IP do map-server #%d atualizado para %d.%d.%d.%d.\n", id, CONVIP(server[id].ip));
			RFIFOSKIP(fd,6);
		break;
		...
		default:
		{
			...
			ShowError("Pacote 0x%04x desconhecido recebido do map-server, desconectando.\n", RFIFOW(fd,0));
			set_eof(fd);
			return 0;
		}
		} // switch
...
int lan_subnetcheck(uint32 ip)
{
	...
	if( i < subnet_count ) {
		ShowInfo("Checagem de sub-rede [%u.%u.%u.%u]: Corresponde a "CL_CYAN"%u.%u.%u.%u/%u.%u.%u.%u"CL_RESET"\n", CONVIP(ip), CONVIP(subnet[i].char_ip & subnet[i].mask), CONVIP(subnet[i].mask));
		return subnet[i].map_ip;
	} else {
		ShowInfo("Checagem de sub-rede [%u.%u.%u.%u]: "CL_CYAN"WAN"CL_RESET"\n", CONVIP(ip));
		return 0;
	}
}
...
{// CH: <0829>.W <char id>.L <birth date:YYMMDD>.6B
	...
	ShowInfo(CL_RED"Requisição de deleção de personagem: "CL_GREEN"%d (%d)"CL_RESET"\n", sd->account_id, char_id);
...
int parse_char(int fd)
{
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			ShowInfo("Requisição de conexão - account_id:%d / login_id1:%d / login_id2:%d\n", account_id, login_id1, login_id2);
			...
			FIFOSD_CHECK(3);
		{
			...
			if (i < 0 || !cd->last_point.map) {
				...
				if (j == ARRAYLENGTH(server)) {
					ShowInfo("Conexão fechada. Não há map-servers disponíveis.\n");
					WFIFOHEAD(fd,3);
					WFIFOW(fd,0) = 0x81;
				...
				if ((i = search_mapserver((j=mapindex_name2id(MAP_PRONTERA)),-1,-1)) >= 0) {
					...
					ShowInfo("Conexão fechada. Não há map-server que contenha uma capital, e incapaz de encontrar um map-server para '%s'.\n", mapindex_id2name(cd->last_point.map));
					WFIFOHEAD(fd,3);
					WFIFOW(fd,0) = 0x81;
					WFIFOB(fd,2) = 1; // 01 = Server closed
					WFIFOSET(fd,3);
				...
				ShowWarning("Incapaz de encontrar map-server para '%s', mandando jogador para capital '%s'.\n", mapindex_id2name(cd->last_point.map), mapindex_id2name(j));
				cd->last_point.map = j;
			}
			...
			{
				ShowError("parse_char: Tentativa de escrever em sessão inválida %d! Map-server #%d desconectado.\n", map_fd, i);
				server[i].fd = -1;
...
int parse_console(const char* command)
{
	...
	if( strcmpi("shutdown", command) == 0 || strcmpi("exit", command) == 0 || strcmpi("quit", command) == 0 || strcmpi("end", command) == 0 || strcmpi("fim", command) == 0 || strcmpi("finalizar", command) == 0)
		runflag = 0;
	...
	else if( strcmpi("help", command) == 0 || strcmpi("ajuda", command) == 0)
	{
		...
		ShowInfo("  'shutdown | exit | quit  |end | fim | finalizar'\n");
		ShowInfo("Saber se o servidor está ativo:\n");
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		if(strcmpi(w1,"timestamp_format") == 0) {
			...
			if (login_ip) {
				...
				ShowStatus("Endereço IP do login-server: %s -> %s\n", w2, ip2str(login_ip, ip_str));
			}
			...
			if (char_ip){
				...
				ShowStatus("Endereço IP do char-server: %s -> %s\n", w2, ip2str(char_ip, ip_str));
			}
			...
			if (bind_ip) {
				...
				ShowStatus("IP do bind do char-server: %s -> %s\n", w2, ip2str(bind_ip, ip_str));
			}
			...
			if (!start_point.map)
				ShowError("start_point especificado %s não foi encontrado no map-index.\n", map);
			start_point.x = x;
			start_point.y = y;
			...
			if( char_per_account == 0 || char_per_account > MAX_CHARS ) {
				if( char_per_account > MAX_CHARS )
					ShowWarning("Máx. de personagens por conta '%d' excedeu o limite. Padronizando em '%d'.\n", char_per_account, MAX_CHARS);
				char_per_account = MAX_CHARS;
			}
			...
			if (fame_list_size_chemist > MAX_FAME_LIST) {
				ShowWarning("Tamanho máx. de fame list é %d (fame_list_alchemist)\n", MAX_FAME_LIST);
			...
			if (fame_list_size_smith > MAX_FAME_LIST) {
				ShowWarning("Tamanho máx. de fame list é %d (fame_list_blacksmith)\n", MAX_FAME_LIST);
			...
			if (fame_list_size_taekwon > MAX_FAME_LIST) {
				ShowWarning("Tamanho máx. de fame list é %d (fame_list_taekwon)\n", MAX_FAME_LIST);
...
int do_init(int argc, char **argv)
{
	...
	if (strcmp(userid, "s1")==0 && strcmp(passwd, "p1")==0) {
		ShowWarning("O uso do login/senha padrões s1/p1 NÃO É RECOMENDADO.\n");
		ShowNotice("Edite a tabela 'login' para criar um login/senha adequado (gênero 'S')\n");
		ShowNotice("Após, mude o login/senha para usar em conf/char_athena.conf (ou conf/import/char_conf.txt)\n");
	}
	...
	if ((naddr_ != 0) && (!login_ip || !char_ip))
	{
		...
		if (naddr_ > 1)
			ShowStatus("Múltiplas interfaces detectadas..  usando %s como nosso endereço IP.\n", ip_str);
		else
			ShowStatus("Definindo %s como nosso endereço IP.\n", ip_str);
		if (!login_ip) {
