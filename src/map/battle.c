...
int battle_attr_fix(struct block_list *src, struct block_list *target, int damage,int atk_elem,int def_type, int def_lv)
{
	...
	if (def_type < 0 || def_type > ELE_MAX ||
		def_lv < 1 || def_lv > 4) {
		ShowError("battle_attr_fix: tipo attr desconhecido: atk=%d def_type=%d def_lv=%d\n",atk_elem,def_type,def_lv);
		return damage;
	}
...
#define MATK_ADD( a ) { ad.damage+= a; }
		...
		{	//Adds atk2 to the damage, should be influenced by number of hits and skill-ratio, but not mdef reductions. [Skotlex]
			...
			if(mflag>0)
				...
				ShowError("Alcance zero por %d:%s, divisão por 0 evitada!\n", skill_id, skill_get_name(skill_id));
		}
...
int battle_set_value(const char* w1, const char* w2)
{
	...
	{
		ShowWarning("Valor para configuração '%s': %s é inválido (min:%i máx:%i)! Padronizando em %i...\n", w1, w2, battle_data[i].min, battle_data[i].max, battle_data[i].defval);
...
void battle_adjust_conf()
{
	...
	if( battle_config.feature_buying_store ) {
		ShowWarning("conf/battle/feature.conf buying_store está ativo, mas requer PACKETVER 2010-04-27 ou mais novo, desabilitando...\n");
	...
	if( battle_config.feature_search_stores ) {
		ShowWarning("conf/battle/feature.conf search_stores está ativo, mas requer PACKETVER 2010-08-03 ou mais novo, disabilitando...\n");
	...
	if (battle_config.cell_stack_limit != 1)
		ShowWarning("Configuração de batalha 'cell_stack_limit' não tem efeito neste servidor, visto que este foi compilado sem suporte a Cell Stack Limit.\n");
#endif
}
