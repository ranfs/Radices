...
bool pc_authok(struct map_session_data *sd, int login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	ShowInfo("'"CL_WHITE"%s"CL_RESET"' entrou."
...
int pc_jobchange(struct map_session_data *sd,int job, int upper)
{
	...
	if( (sd->class_&MAPID_UPPERMASK) == MAPID_STAR_GLADIATOR && (b_class&MAPID_UPPERMASK) != MAPID_STAR_GLADIATOR) {
		// Saindo da linhagem de Taekwon, podemos resetar as variáveis não usadas pelo servidor.
		pc_resetfeel(sd);
	}
...
//			ShowDebug("%s - Class %d: %u\n", type?"Job":"Base", job_id, max_level[job][type]);
	...
	ShowStatus("Finalizada leitura do "CL_WHITE"%s"CL_RESET".\n","exp.txt");
