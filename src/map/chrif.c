...
void chrif_checkdefaultlogin(void) {
	if (strcmp(userid, "s1")==0 && strcmp(passwd, "p1")==0) {
		...
		ShowNotice("Edite sua tabela 'login' para criar um login/senha adequados (gênero 'S').\n");
		ShowNotice("Após, edite seu login/senha em conf/map_athena.conf (ou conf/import/map_conf.txt)\n");
	}
}
...
int chrif_setip(const char* ip) {
	...
	if ( !( char_ip = host2ip(ip) ) ) {
		ShowWarning("Falha em resolver IP do char-server! (%s)\n", ip);
...
int chrif_save(struct map_session_data *sd, int flag) {
	...
	if (flag && sd->state.active) { //Store player data which is quitting
		...
		if ( !chrif_auth_logout(sd,flag == 1 ? ST_LOGOUT : ST_MAPCHANGE) )
			ShowError("chrif_save: Falha em preparar o jogador %d:%d para uma saída adequada!\n", sd->status.account_id, sd->status.char_id);
	}
...
int chrif_removemap(int fd) {
	...
	if(battle_config.etc_log)
		ShowStatus("Removido mapa do servidor %d.%d.%d.%d:%d (%d mapas)\n", CONVIP(ip), port, j);
...
int chrif_sendmapack(int fd) {
	...
	if (RFIFOB(fd,2)) {
		ShowFatalError("chrif : envio da lista de mapas para o char-server falhou %d\n", RFIFOB(fd,2));
		exit(EXIT_FAILURE);
	}
...
void chrif_authok(int fd) {
	...
	if( RFIFOW(fd,2) - 25 != sizeof(struct mmo_charstatus) ) {
		ShowError("chrif_authok: Tamanho das informações diferem! %d != %d\n", RFIFOW(fd,2) - 25, sizeof(struct mmo_charstatus));
		return;
	}
...
int chrif_recvfamelist(int fd) {
	...
	ShowInfo("Recebida lista de ferreiros de '"CL_WHITE"%d"CL_RESET"' personagens.\n", total);
...
int chrif_load_scdata(int fd) {
	...
	if ( sd->status.char_id != cid ) {
		ShowError("chrif_load_scdata: Recebendo dados para conta %d, char_id incompatível (%d != %d)!\n", aid, sd->status.char_id, cid);
		return -1;
	}
...
int chrif_parse(int fd) {
	...
	while ( RFIFOREST(fd) >= 2 ) {
		...
		if (cmd < 0x2af8 || cmd >= 0x2af8 + ARRAYLENGTH(packet_len_table) || packet_len_table[cmd-0x2af8] == 0) {
			...
			ShowWarning("chrif_parse: sessão #%d, intif_parse falhou (comando irreconhecido 0x%.4x).\n", fd, cmd);
			set_eof(fd);
			return 0;
		}
...
static int check_connect_char_server(int tid, unsigned int tick, int id, intptr_t data) {
	...
	if ( char_fd <= 0 || session[char_fd] == NULL ) {
		if ( !displayed ) {
			ShowStatus("Tentando conectar-se ao char-server. Aguarde.\n");
			displayed = 1;
		}
