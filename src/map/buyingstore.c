...
bool buyingstore_setup(struct map_session_data* sd, unsigned char slots)
{
	...
	{
		ShowWarning("buyingstore_setup: Requisitado %d slots, mas o servidor suporta até %d slots.\n", (int)slots, MAX_BUYINGSTORE_SLOTS);
...
void buyingstore_create(struct map_session_data* sd, int zenylimit, unsigned char result, const char* storename, const uint8* itemlist, unsigned int count)
{
	...
	{// itemlist: <name id>.W <amount>.W <price>.L
		...
		{// duplicate check. as the client does this too, only malicious intent should be caught here
			...
			if( listidx != i )
			{// duplicate
				ShowWarning("buyingstore_create: Encontrada duplicata de item na lista de compra (nameid=%hu, quant.=%hu, account_id=%d, char_id=%d).\n", nameid, amount, sd->status.account_id, sd->status.char_id);
				break;
			}
		}
...
void buyingstore_trade(struct map_session_data* sd, int account_id, unsigned int buyer_id, const uint8* itemlist, unsigned int count)
{
	...
	{// itemlist: <index>.W <name id>.W <amount>.W
		...
		{// duplicate check. as the client does this too, only malicious intent should be caught here
			...
			if( k != i )
			{// duplicate
				ShowWarning("buyingstore_trade: Encontrada duplicata de item na lista de venda (nameidante=%hu, quant.ante=%hu, nameid=%hu, quant.=%hu, account_id=%d, char_id=%d).\n",
