...
ACMD_FUNC(monster)
{
	...
	if (battle_config.etc_log)
		ShowInfo("%s monstro='%s' nome='%s' id=%d quant.=%d (%d,%d)\n", command, monster, name, mob_id, number, sd->bl.x, sd->bl.y);
		...
		if (i == AUTOLOOTITEM_SIZE) {
			...
			for(i = 0; i < AUTOLOOTITEM_SIZE; i++)
			{
				...
				if (!(item_data = itemdb_exists(sd->state.autolootid[i]))) {
					ShowDebug("Item inexistente %d na lista de autolootitem (account_id: %d, char_id: %d)", sd->state.autolootid[i], sd->status.account_id, sd->status.char_id);
					continue;
				}
...
ACMD_FUNC(mobinfo)
{
	unsigned char msize[3][9] = {"Pequeno", "Médio", "Grande"};
	unsigned char mrace[12][11] = {"Formless", "Undead", "Beast", "Plant", "Insect", "Fish", "Demon", "Demi-Human", "Angel", "Dragon", "Boss", "Non-Boss"};
	unsigned char melement[10][12] = {"Neutro", "Água", "Terra", "Fogo", "Ar", "Veneno", "Sagrado", "Escuro", "Fantasma", "Morto-vivo"};
...
#define setflag( cmd ) \
	if ( strcmp( flag_name , #cmd ) == 0 ){\
		...
		sprintf(atcmd_output,"[ @mapflag ] %s foi %s para o valor = %hd",#cmd,flag?"ligada":"desligada",flag);\
		clif_displaymessage(sd->fd,atcmd_output);\
		return 0;\
	}
        char flag_name[100];
...
void atcommand_basecommands(void) {
	...
	for( i = 0; i < ARRAYLENGTH(atcommand_base); i++ ) {
		if(atcommand_exists(atcommand_base[i].command)) { // Should not happen if atcommand_base[] array is OK
			ShowDebug("atcommand_basecommands: duplicata ACMD_DEF para '%s'.\n", atcommand_base[i].command);
			continue;
		}
...
static void atcommand_config_read(const char* config_filename)
{
	...
	if (aliases != NULL) {
		...
		for (i = 0; i < count; ++i) {
			...
			if (!atcommand_exists(commandname)) {
				ShowConfigWarning(command, "atcommand_config_read: não pôde definir pseudônimo para comando inexistente %s", commandname);
				continue;
			}
			...
			for (j = 0; j < alias_count; ++j) {
				...
				if (alias != NULL) {
					...
					if (strdb_exists(atcommand_alias_db, alias)) {
						ShowConfigWarning(command, "atcommand_config_read: pseudônimo %s já existe", alias);
						continue;
					}
	...
	if (help != NULL) {
		...
		for (i = 0; i < count; ++i) {
			...
			commandname = config_setting_name(command);
			if (!atcommand_exists(commandname))
				ShowConfigWarning(command, "atcommand_config_read: comando %s não existe", commandname);
		}
	}
