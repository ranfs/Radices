...
int login_lan_config_read(const char *lancfgName)
{
	...
	if((fp = fopen(lancfgName, "r")) == NULL) {
		ShowWarning("Arquivo de configuração de LAN não foi encontrado: %s\n", lancfgName);
		return 1;
	}
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(sscanf(line,"%[^:]: %[^:]:%[^:]:%[^\r\n]", w1, w2, w3, w4) != 4)
		{
			ShowWarning("Erro na sintaxe no arquivo de configuração %s na linha %d.\n", lancfgName, line_num);
			continue;
		}
		...
		if( strcmpi(w1, "subnet") == 0 )
		{
			...
			{
				ShowError("%s: Erro de configuração: O char-server (%s) e map-server (%s) pertecem a diferentes sub-redes!\n", lancfgName, w3, w4);
				continue;
			}
...
int parse_console(const char* command)
{
	...
	if( strcmpi("shutdown", command) == 0 || strcmpi("exit", command) == 0 || strcmpi("quit", command) == 0 || strcmpi("end", command) == 0 || strcmpi("fim", command) == 0 || strcmpi("sair", command) == 0 )
		runflag = 0;
	else if( strcmpi("alive", command) == 0 || strcmpi("status", command) == 0 )
		ShowInfo(CL_CYAN"Console: "CL_BOLD"Estou Vivo."CL_RESET"\n");
	else if( strcmpi("help", command) == 0 || strcmpi("ajuda", command) == 0)
	{
		ShowInfo("Para desligar o servidor:\n");
		ShowInfo("  'shutdown|exit|quit|end|fim|sair'\n");
		ShowInfo("Para saber se o servidor está ativo:\n");
		ShowInfo("  'alive|status'\n");
		ShowInfo("Para criar uma nova conta:\n");
		ShowInfo("  'create|criar'\n");
	}
	else
	{// commands with parameters
		...
		if( strcmpi(cmd, "create") == 0 || strcmpi(cmd, "criar") == 0)
		{
			...
			{
				ShowWarning("Console: Parâmetros inválidos para '%s'. Uso: %s <usuário> <senha> <sexo:F/M>\n", cmd, cmd);
				return 0;
			}

			if( mmo_auth_new(username, password, TOUPPER(sex), "0.0.0.0") != -1 )
			{
				ShowError("Console: Falha na criação do personagem.\n");
				return 0;
			}
...
int parse_fromchar(int fd)
{
	...
	{// not a char server
		ShowDebug("parse_fromchar: Desconectando sessão inválida #%d (não é um char-server)\n", fd);
		set_eof(fd);
		do_close(fd);
		return 0;
	}
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			{// authentication not found
				ShowStatus("Char-server '%s': autenticação da conta %d "CL_RED"RECUSADA"CL_RESET" (ip: %s).\n", server[id].name, account_id, ip);
				WFIFOHEAD(fd,25);
				WFIFOW(fd,0) = 0x2713;
				WFIFOL(fd,2) = account_id;
				WFIFOL(fd,6) = login_id1;
				...
				return 0;
		{
			...
			{
				ShowStatus("Quant. de usuários no %s : %d\n", server[id].name, users);
				...
				return 0;
		{
			...
			if( e_mail_check(email) == 0 )
				ShowNotice("Char-server '%s': Tentativa de criar um email em uma conta com email padrão "CL_RED"RECUSADA"CL_RESET" - email é inválido (conta: %d, ip: %s)\n", server[id].name, account_id, ip);
			else
			if( !accounts->load_num(accounts, &acc, account_id) || strcmp(acc.email, "a@a.com") == 0 || acc.email[0] == '\0' )
				ShowNotice("Char-server '%s': Tentativa de criar um email em uma conta com email padrão "CL_RED"RECUSADA"CL_RESET" - conta não existe ou email da conta Não é o email padrão (conta: %d, ip: %s).\n", server[id].name, account_id, ip);
			else {
				memcpy(acc.email, email, 40);
				ShowNotice("Char-server '%s': Criação de um email em uma conta com email padrão (conta: %d, novo email: %s, ip: %s).\n", server[id].name, account_id, email, ip);
				// Save
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': conta %d "CL_RED"NÃO"CL_RESET" encontrada (ip: %s).\n", server[id].name, account_id, ip);
			else
			{
				...
				return 0;
		{
			...
			if( e_mail_check(actual_email) == 0 )
				ShowNotice("Char-server '%s': Tentativa de mudar email em uma conta (comando @email), mas email atual é inválido (conta: %d, ip: %s)\n", server[id].name, account_id, ip);
			else
			if( e_mail_check(new_email) == 0 )
				ShowNotice("Char-server '%s': Tentativa de mudar email em uma conta (comando @email) com um novo email inválido (conta: %d, ip: %s)\n", server[id].name, account_id, ip);
			else
			if( strcmpi(new_email, "a@a.com") == 0 )
				ShowNotice("Char-server '%s': Tentativa de mudar email em uma conta (comando @email) com o email padrão (conta: %d, ip: %s)\n", server[id].name, account_id, ip);
			else
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': Tentativa de mudar email em uma conta (comando @email), mas conta não existe (conta: %d, ip: %s).\n", server[id].name, account_id, ip);
			else
			if( strcmpi(acc.email, actual_email) != 0 )
				ShowNotice("Char-server '%s': Tentativa de mudar email em uma conta (comando @email), mas email atual é incorreto (conta: %d (%s), email atual: %s, email proposto: %s, ip: %s).\n", server[id].name, account_id, acc.userid, acc.email, actual_email, ip);
			else {
				safestrncpy(acc.email, new_email, 40);
				ShowNotice("Char-server '%s': Modificação de um email em uma conta (comando @email) (conta: %d (%s), novo email: %s, ip: %s).\n", server[id].name, account_id, acc.userid, new_email, ip);
				// Save
				accounts->save(accounts, &acc);
			}
		}
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': Erro em mudança de status (conta: %d não encontrada, status sugerido %d, ip: %s).\n", server[id].name, account_id, state, ip);
			else
			if( acc.state == state )
				ShowNotice("Char-server '%s': Erro em mudança de status - status atual já é o status bom (conta: %d, status %d, ip: %s).\n", server[id].name, account_id, state, ip);
			else {
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': Erro em requisição de ban (conta: %d não encontrada, ip: %s).\n", server[id].name, account_id, ip);
			else
			{
				time_t timestamp;
				struct tm *tmtime;
				...
				if (timestamp == -1)
					ShowNotice("Char-server '%s': Erro em requisição de ban (conta: %d, data inválida, ip: %s).\n", server[id].name, account_id, ip);
				else
				if( timestamp <= time(NULL) || timestamp == 0 )
					ShowNotice("Char-server '%s': Erro em requisição de ban (conta: %d, nova data des-bane a conta, ip: %s).\n", server[id].name, account_id, ip);
				else
				{
					uint8 buf[11];
					char tmpstr[24];
					timestamp2string(tmpstr, sizeof(tmpstr), timestamp, login_config.date_format);
					ShowNotice("Char-server '%s': Requisição de ban (conta: %d, nova data de banimento: %d (%s), ip: %s).\n", server[id].name, account_id, timestamp, tmpstr, ip);
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': Erro em mudança de sexo (conta: %d não encontrada, ip: %s).\n", server[id].name, account_id, ip);
			else
			if( acc.sex == 'S' )
				ShowNotice("Char-server '%s': Erro em mudança de sexo - conta a mudar é do tipo Servidor (conta: %d, ip: %s).\n", server[id].name, account_id, ip);
			else
			{
				unsigned char buf[7];
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowStatus("Char-server '%s': recebendo (do char-server) account_reg2 (conta: %d não encontrada, ip: %s).\n", server[id].name, account_id, ip);
			else
			{
				int len;
				int p;
				ShowNotice("char-server '%s': recebendo (do char-server) account_reg2 (conta: %d, ip: %s).\n", server[id].name, account_id, ip);
				...
				return 0;
		{
			...
			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowNotice("Char-server '%s': Erro em requisição de unban (conta: %d não encontrada, ip: %s).\n", server[id].name, account_id, ip);
			else
			if( acc.unban_time == 0 )
				ShowNotice("Char-server '%s': Erro em requisição de unban (conta: %d, sem mudança na data de unban, ip: %s).\n", server[id].name, account_id, ip);
			else
			{
				ShowNotice("Char-server '%s': Requisição de unban (conta: %d, ip: %s).\n", server[id].name, account_id, ip);
				acc.unban_time = 0;
				accounts->save(accounts, &acc);
			}
		}
		...
		case 0x2737: //Request to set all offline.
			ShowInfo("Definindo contas do char-server %d offline.\n", id);
			online_db->foreach(online_db, online_db_setoffline, id);
			RFIFOSKIP(fd,2);
		break;

		default:
			ShowError("parse_fromchar: Pacote 0x%x "CL_RED"desconhecido"CL_RESET" recebido de um char-server! Desconectando!\n", command);
			set_eof(fd);
			return 0;
		} // switch
	} // while
...
int mmo_auth_new(const char* userid, const char* pass, const char sex, const char* last_ip) {
	...
	if( DIFF_TICK(tick, new_reg_tick) < 0 && num_regs >= allowed_regs ) {
		ShowNotice("Registro de conta negada (limite de registros execedeu o limite)\n");
		return 3;
	}
	...
	if( accounts->load_str(accounts, &acc, userid) ) {
		ShowNotice("Tentativa de criação de uma conta pré-existente (conta: %s_%c, senha: %s, senha recebida: %s)\n", userid, sex, acc.pass, pass);
...
int mmo_auth(struct login_session_data* sd, bool isServer) {
	...
	if( login_config.use_dnsbl ) {
		...
		for( dnsbl_serv = strtok(login_config.dnsbl_servs,","); dnsbl_serv != NULL; dnsbl_serv = strtok(NULL,",") ) {
			...
			if( host2ip(ip_dnsbl) ) {
				ShowInfo("DNSBL: (%s) "CL_RED"na lista negra"CL_RESET". Usuário kikado.\n", r_ip);
				return 3;
			}
		}
	...
	if( !check_password(sd->md5key, sd->passwdenc, sd->passwd, acc.pass) ) {
		ShowNotice("Senha inválida (conta: '%s', senha: '%s', senha recebida: '%s', ip: %s)\n", sd->userid, acc.pass, sd->passwd, ip);
		return 1; // 1 = Incorrect Password
	}
	...
	if( login_config.client_hash_check && !isServer ) {
		...
		if( !sd->has_client_hash ) {
			ShowNotice("Client não enviou hash (conta: %s, senha: %s, ip: %s)\n", sd->userid, sd->passwd, acc.state, ip);
			return 5;
		}
		...
		if( !match ) {
			...
			ShowNotice("Hash de client inválido (conta: %s, senha: %s, MD5 enviado: %d, ip: %s)\n", sd->userid, sd->passwd, smd5, ip);
			return 5;
		}
	}
	...
	if( sd->sex != 'S' && sd->account_id < START_ACCOUNT_NUM )
		ShowWarning("Conta %s tem ID de conta %d! IDs de conta devem ser superiores que %d para funcionar corretamente!\n", sd->userid, sd->account_id, START_ACCOUNT_NUM);
...
void login_auth_ok(struct login_session_data* sd)
{
	...
	if( login_config.group_id_to_connect >= 0 && sd->group_id != login_config.group_id_to_connect ) {
		ShowStatus("Conexão recusada: o group_id requerido para logar é %d (conta: %s, grupo: %d).\n", login_config.group_id_to_connect, sd->userid, sd->group_id);
		WFIFOHEAD(fd,3);
		WFIFOW(fd,0) = 0x81;
		...
		ShowStatus("Conexão recusada: o menor group_id requerido para logar é %d (conta: %s, grupo: %d).\n", login_config.min_group_id_to_connect, sd->userid, sd->group_id);
		WFIFOHEAD(fd,3);
		WFIFOW(fd,0) = 0x81;
		WFIFOB(fd,2) = 1; // 01 = Server closed
		WFIFOSET(fd,3);
	...
	{// if no char-server, don't send void list of servers, just disconnect the player with proper message
		ShowStatus("Conexão recusada: não há char-server ativo (conta: %s).\n", sd->userid);
		WFIFOHEAD(fd,3);
		WFIFOW(fd,0) = 0x81;
		WFIFOB(fd,2) = 1; // 01 = Server closed
		WFIFOSET(fd,3);
...
void login_auth_failed(struct login_session_data* sd, int result)
{
	...
	if (login_config.log_login)
	{
		...
		case   5: error = "Jogo desatualizado."; break; // 5 = Your Game's EXE file is not the latest version
		...
		case   8: error = "Limite de conexões atingido."; break; // 8 = No more accounts may be connected from this company
		...
		case  12: error = "Trabalhando na DB"; break; // 12 = MSI_REFUSE_TEMP_BAN_FOR_DBWORK
		case  13: error = "Travamento próprio"; break; // 13 = MSI_REFUSE_SELF_LOCK
		...
		case  99: error = "Conta sumiu."; break; // 99 = This ID has been totally erased
		case 100: error = "Informações de login continuam."; break; // 100 = Login information remains at %s
		case 101: error = "Investigando hack."; break; // 101 = Account has been locked for a hacking investigation. Please contact the GM Team for more information
		case 102: error = "Investigando bug."; break; // 102 = This account has been temporarily prohibited from login due to a bug-related investigation
		...
		case 104: error = "Deleting esposa do personagem."; break; // 104 = This character is being deleted. Login is temporarily unavailable for the time being
...
int parse_login(int fd)
{
	...
	if( session[fd]->flag.eof )
	{
		ShowInfo("Conexão de '"CL_WHITE"%s"CL_RESET"' desligada.\n", ip);
		do_close(fd);
		return 0;
	}
	...
	if( sd == NULL )
	{
		...
		{
			ShowStatus("Conexão recusada: IP não é autorizado (deny/allow, ip: %s).\n", ip);
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			ShowInfo("Requisição de conexão do char-server '%s' em %u.%u.%u.%u:%u (conta: '%s', senha: '%s', ip: '%s')\n", server_name, CONVIP(server_ip), server_port, sd->userid, sd->passwd, ip);
			...
			{
				ShowNotice("Conexão do char-server '%s' "CL_RED"RECUSADA"CL_RESET".\n", server_name);
				WFIFOHEAD(fd,3);
				WFIFOW(fd,0) = 0x2711;
				WFIFOB(fd,2) = 3;
				WFIFOSET(fd,3);
		...
		default:
			ShowNotice("Fim anormal de conexão (ip: %s): Pacote 0x%x desconhecido.\n", ip, command);
			set_eof(fd);
			return 0;
		}
	}
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		if(!strcmpi(w1,"timestamp_format"))
			...
			if( msg_silent ) /* only bother if we actually have this enabled */
				ShowInfo("Silenciosidade: %d\n", atoi(w2));
		}
		else if( !strcmpi(w1, "bind_ip") ) {
			char ip_str[16];
			login_config.login_ip = host2ip(w2);
			if( login_config.login_ip )
				ShowStatus("IP do bind do login-server: %s -> %s\n", w2, ip2str(login_config.login_ip, ip_str));
		}
...
int do_init(int argc, char** argv)
{
	...
	if( accounts == NULL ) {
		ShowFatalError("do_init: motor de contas '%s' não encontrado.\n", login_config.account_engine);
		exit(EXIT_FAILURE);
	} else {

		if(!accounts->init(accounts)) {
			ShowFatalError("do_init: Falha em inicializar motor de contas '%s'.\n", login_config.account_engine);
			exit(EXIT_FAILURE);
		}
	}
